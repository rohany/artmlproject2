...
# trainAB = load_data('better-dataset/train/*.jpg')
# valAB = load_data('better-dataset/val/*.jpg')
...
#     if epoch != old_epoch:
#         netG.save("weights_at_epoch-" + str(old_epoch) + ".weights")
...
# netG.save("training_done_weights50.weights")
# _, valA, valB = train_batch.send(6) 
# fakeB = netG_gen(valA)
# showX(np.concatenate([valA, valB, fakeB], axis=0), 3)
# errL1_sum = errG_sum = errD_sum = 0
...
#     _, valA, valB = train_batch.send(1)
#     gen = netG_generate([valA])
#     myShow(np.asarray([gen]), "firstrun-output/res-" + str(i) + ".jpg")

# TODO: load the network weights, take in an input image, prepare it, run through network, display and save?
...
netG.load_weights(weights)

def process_image(image):
    im = cv.imread(image, cv.IMREAD_COLOR)
    im = cv.resize(im, (imageSize, imageSize))
    arr = np.array(im).astype(float)
    arr = arr/255*2-1
    img = arr[:, :, :]
    return img
...
gen = netG_generate([np.float32([img])])
myShow(np.asarray([gen]), out_path)
